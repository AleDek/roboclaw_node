...
class EncoderOdom:
    ...
    def update(self, enc_left, enc_right):
        ...
        if left_ticks == right_ticks:
            ...
            self.cur_theta = self.normalize_angle(self.cur_theta)
...
class Node:
    ...
    def run(self):
        ...
        #set_cmd_thread = Thread(target=self.set_cmd,args="")
        #set_cmd_thread.start()
        while not rospy.is_shutdown():
            ...
            #self.roboclaw.SpeedM1M2(self.address, 150, 150)
            
            try:
                with self.mutex:
                    status1, enc1, crc1 = self.roboclaw.ReadEncM1(self.address)
                    print("Letto enc1: %d", enc1)
            except ValueError:
                pass
            except OSError as e:
            ...
            if ((isinstance(enc1,Number) and isinstance(enc2,Number))):
                ...
                self.roboclaw.SpeedM1M2(self.address, self.vl_ticks, self.vr_ticks)
    ...
    def set_cmd(self):
        ...
        cmd_set_time_rate = rospy.Rate(200)
        while not rospy.is_shutdown():
                print("qui!")
                self.roboclaw.SpeedM1M2(self.address, 0, 0)
                cmd_set_time_rate.sleep()
    ...
    def cmd_vel_callback(self, twist):
        ...
        self.vr_ticks = int(vr * self.TICKS_PER_METER)  # ticks/s
        self.vl_ticks = int(vl * self.TICKS_PER_METER)

        print("Ticks: ", self.vr_ticks, self.vl_ticks)
        #rospy.loginfo("vr_ticks:%8d vl_ticks: %8d", self.vr_ticks, self.vl_ticks)
